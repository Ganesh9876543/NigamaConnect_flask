...
def determine_relationship(member: Dict[str, Any], target_node: Dict[str, Any], members_dict: Dict[str, Dict[str, Any]]) -> str:
    ...
    """
    # Debug print
    print(f"Determining relationship for member {member.get('id')} to target {target_node.get('id')}")
    print(f"Target parentId: {target_node.get('parentId')}, Member ID: {member.get('id')}")
    print(f"Member gender: {member.get('gender')}")
    
    # First check if member is the target node itself
    if member.get('id') == target_node.get('id'):
        return "Myself"
    
    # Check if member is target's parent - this is crucial, ensure it's working
    parent_id = target_node.get('parentId')
    if parent_id and parent_id == member.get('id'):
        # Explicitly check the gender string
        gender = member.get('gender', '').lower()
        if gender == 'male':
            print(f"FOUND FATHER: {member.get('name')}")
            return "Father"
        elif gender == 'female':
            print(f"FOUND MOTHER: {member.get('name')}")
            return "Mother"
        else:
            print(f"FOUND PARENT (unknown gender {gender}): {member.get('name')}")
            return "Parent"
    ...
    if parent_id and parent_id in members_dict:
        ...
        if parent.get('spouse') == member.get('id'):
            gender = member.get('gender', '').lower()
            if gender == 'male':
                print(f"FOUND FATHER (parent's spouse): {member.get('name')}")
                return "Father"
            elif gender == 'female':
                print(f"FOUND MOTHER (parent's spouse): {member.get('name')}")
                return "Mother"
            else:
                return "Parent"
    ...
    # If we've reached here, print debug info
    print(f"Falling back to 'Relative' for member: {member.get('name')}")
    print(f"Member attributes: id={member.get('id')}, gender={member.get('gender')}, parentId={member.get('parentId')}")
    
    # Default
    return "Relative"

def debug_relationships(family_members: List[Dict[str, Any]], node_id: str) -> None:
    """
    Debug function to print all relationships for a given node.
    Especially useful for checking why mother relationships might not be detected.
    ...
    print(f"\n===== DEBUG RELATIONSHIPS FOR NODE {node_id} =====")
    print(f"Target node data: {target_node}")
    
    # Check parent information
    parent_id = target_node.get('parentId')
    if parent_id:
        if parent_id in members_dict:
            parent = members_dict[parent_id]
            print(f"Parent found: ID={parent_id}, Name={parent.get('name')}, Gender={parent.get('gender')}")
            
            # Check if parent has a spouse (potential other parent)
            spouse_id = parent.get('spouse')
            if spouse_id and spouse_id in members_dict:
                spouse = members_dict[spouse_id]
                print(f"Parent's spouse found: ID={spouse_id}, Name={spouse.get('name')}, Gender={spouse.get('gender')}")
        else:
            print(f"Parent with ID {parent_id} referenced but not found in data!")
    else:
        print("No parent ID set for this node")
    
    # List all female members to check if any could be the mother
    print("\nAll female members in data:")
    for member_id, member in members_dict.items():
        if member.get('gender') == 'female':
            print(f"Female member: ID={member_id}, Name={member.get('name')}, "
                  f"Is parent: {member_id == parent_id}, "
                  f"Is spouse of parent: {parent_id and parent_id in members_dict and member_id == members_dict[parent_id].get('spouse')}")
...
def get_related_nodes(family_members: List[Dict[str, Any]], node_id: str) -> List[Dict[str, Any]]:
    ...
    """
    # Debug to check why mother might not be detected
    debug_relationships(family_members, node_id)
    ...
    for node_id_rel in related_node_ids:
        if node_id_rel in members_dict:
            ...
            # (it's already in the member data, but we make sure it's there)
            spouse_node_id = member.get('spouse')
            if spouse_node_id:
                # Spouse ID is already in the member data under 'spouse'
                # Make sure it's not removed when copying
    ...
    if node_id in members_dict:
        ...
        if spouse_node_id:
            # Spouse ID is already in the self_node data
            # Add spouse's name for convenience
            if spouse_node_id in members_dict:
                spouse_name = members_dict[spouse_node_id].get('name')
                if spouse_name:
...
def get_extended_family(family_tree_id: str, node_id: str, db=None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Use the helper function to get related nodes
        result = get_related_nodes(family_members, node_id)
        
        return result
        
    except Exception as e:
        print(f"Error retrieving extended family: {e}")
        return [] 